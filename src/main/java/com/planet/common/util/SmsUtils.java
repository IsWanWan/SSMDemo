...
public class SmsUtils {
    ...
    public static String sendMobileSms(String mobile) throws IOException, URISyntaxException {
        ...
        String num = String.valueOf(ne.nextInt(9999 - 1000 + 1) + 1000);//为变量赋随机值1000-9999

        String tpl_value = URLEncoder.encode("#code#", ENCODING) + "="
                + URLEncoder.encode(num, ENCODING) + "&"
    ...
    public static void main(String[] args) {
        String apikey = "dc1579f1c0d69634115334a906b8e602";  // 云片网的key
        String code =String.valueOf( (int)(Math.random()*9000+1000)) ;  // 随机生成的四位数验证码
        String mobile = "15921961580";
        System.out.println(" 您的验证码为：" +code +" 手机号：" +mobile);

        String text =" 【超级码力】您的验证码是"+code+",请在5分钟内使用";


        String result = null;  //调用发送短信的 接口
        try {
            result = SmsUtils.sendSms(apikey,text,mobile);
        } catch (IOException e) {
            e.printStackTrace();
        }
